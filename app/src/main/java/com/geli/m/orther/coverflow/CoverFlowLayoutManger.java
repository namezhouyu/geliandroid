package com.geli.m.orther.coverflow;
 ...
 *
 *
 * 如果使用:没有层叠--有间隔--中间的突起--一个界面中只显示3个控件
 * 设置如下:
 * 1、设置ViewHolder为界面的三分一
 * 2、设置mItemSpace大于0 -- 如果不是显示三个，还要微调关于这部分代码
 *
 * 如果是层叠，且中间突起，必须自定义:RecyclerView :如CoverFlowRecyclerView
 *
...
public class CoverFlowLayoutManger extends RecyclerView.LayoutManager {
    ...
    /**是否为平面滚动，Item之间没有叠加，也没有缩放 -- true:平面滚动；false:叠加缩放滚动*/
    ...
    public CoverFlowLayoutManger(boolean isFlat, boolean isGreyItem,
                                 boolean isAlphaItem, float cstInterval, int itemSpace) {
        mIsFlatFlow = isFlat;
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (getItemCount() <= 0 || state == null || state.isPreLayout()) {
            mOffsetAll = 0;
            return;
        }
...
//            cm.setSaturation(0.9f);
    ...
    private float getMaxOffset() {
        if(mItemSpace > 0){
            if(getItemCount() == 1){
                ...
                return (getItemCount() - 2) * getIntervalDistance();
            }else if(getItemCount() > 2){
                return (getItemCount() - 3) * getIntervalDistance();
            }
        }
    ...
    private float computeAlpha(int x) {
        float alpha = 1 - Math.abs(x) * 1.0f / Math.abs(mDecoratedChildWidth / mIntervalRatio);
    ...
    private float getIntervalDistance() {
        if(mItemSpace != 0){
            return mDecoratedChildWidth + mItemSpace;
        }else {
    ...
    public static class Builder {
        ...
        public CoverFlowLayoutManger build() {
            return new CoverFlowLayoutManger(isFlat, isGreyItem,
                    isAlphaItem, cstIntervalRatio, itemSpace);
        }
    }
}