...
public class CustomProgressBar extends View {
    ...
    public CustomProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            mBorderColor = a.getColor(
                    R.styleable.CustomProgressBar_borderColor,
                    DEFAULT_PROGRESS_COLOR);
    ...
    private void drawProgress(Canvas canvas) {
        ...
        if (scale <= 1) {
            ...
            float y = (float) (Math.pow(Math.pow(mRadius, 2) - Math.pow((a - mRadius), 2), 0.5)
                    + bottom - mRadius);
            path.lineTo(left + a, y);
            path.arcTo(new RectF(left, bottom - 2 * mRadius, left + 2 * mRadius, bottom),
                    180 - (float) (angle * 180 / Math.PI),
